...
class MindMapWidget(QWidget):
    ...
    def __init__(self, db_manager, parent=None):
        ...
        self.node_padding = 5 # Default padding
    ...
    def _layout_nodes(self):
        ...
        # Vertical spacing
        y_step = self.height() / (max_level + 2) if max_level > 0 else self.height() / 2
        
        # Horizontal spacing will be dynamic
        
        for level in sorted(level_nodes.keys()):
            nodes_at_level = level_nodes[level]
            
            # Calculate total width needed for nodes at this level
            total_nodes_width = sum(self.notes[node_id]['size'][0] for node_id in nodes_at_level)
            
            # Calculate available horizontal space
            available_width = self.width() - (len(nodes_at_level) + 1) * self.node_padding
            
            # If total_nodes_width is too large, scale down or adjust spacing
            if total_nodes_width > available_width:
                # Simple scaling for now, can be improved
                scale_factor = available_width / total_nodes_width
                current_x = self.node_padding
                for node_id in nodes_at_level:
                    node_width, node_height = self.notes[node_id]['size']
                    node_width *= scale_factor
                    node_height *= scale_factor # Scale height too for consistency
                    self.notes[node_id]['size'] = (node_width, node_height) # Update size
                    
                    x = current_x + node_width / 2
                    y = (level + 1) * y_step # +1 to avoid placing at very top edge
                    self.notes[node_id]['pos'] = QPointF(x, y)
                    current_x += node_width + self.node_padding
            else:
                # Distribute nodes evenly
                current_x = (self.width() - total_nodes_width - (len(nodes_at_level) - 1) * self.node_padding) / 2
                for node_id in nodes_at_level:
                    node_width, node_height = self.notes[node_id]['size']
                    x = current_x + node_width / 2
                    y = (level + 1) * y_step
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            for note_id, data in self.notes.items():
                if data['rect'].contains(event.pos() / self.zoom_factor): # Scale mouse position
