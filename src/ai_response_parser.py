...
class AiResponseParser:
    ...
    strip_qualifiers = staticmethod(strip_qualifiers)
    normalize_tokens = staticmethod(normalize_tokens)
    char_ngram_overlap = staticmethod(char_ngram_overlap)
    GENERIC_TITLE_WORDS = GENERIC_TITLE_WORDS
    is_duplicate_concept = staticmethod(is_duplicate_concept)
    ...
    sanitize_latex_escapes = staticmethod(sanitize_latex_escapes)
    repair_truncated_json = staticmethod(repair_truncated_json)
    ...
    def extract_body_paragraphs(content: str) -> List[str]:
        ...
        for l in lines:
            ...
            if in_related:
                continue
            if l.strip().startswith("# ") and not any(b.strip() for b in body_lines):
