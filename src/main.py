...
import markdown
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QTextEdit, QPushButton, QMainWindow, QAction, QListWidget, QSplitter, QMessageBox, QInputDialog, QHBoxLayout, QComboBox, QStyle, QMenu, QProgressDialog, QDialog, QLabel, QLineEdit, QListWidgetItem 
...
class NoteSelectionDialog(QDialog):
    def __init__(self, db_manager, current_note_id, parent=None):
        ...
        self.setWindowTitle("Select Note to Link")
    ...
    def init_ui(self):
        ...
        self.main_layout.addWidget(self.notes_list_widget)
    ...
    def on_note_selected(self, item):
        self.selected_note_id = item.data(Qt.UserRole)
        self.selected_note_title = item.text()
...
class ZettelkastenApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.link_note_button.clicked.connect(self.link_note_action)
        self.button_layout.addWidget(self.link_note_button)
        ...
        self.linked_notes_list_widget.itemClicked.connect(self.open_selected_note_from_link)
        self.linked_notes_list_widget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.linked_notes_list_widget.customContextMenuRequested.connect(self._show_linked_note_context_menu)
        self.editor_preview_splitter.addWidget(self.linked_notes_list_widget)
    ...
    def _show_note_context_menu(self, position):
        ...
        if item:
            ...
            link_action = context_menu.addAction("Link to...")
    ...
    def link_note_action(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            target_note_id = dialog.selected_note_id
            target_note_title = dialog.selected_note_title
    ...
    def create_new_category(self):
        ...
        if ok and category_name:
            ...
            self.load_notes(category_to_select=category_name)
    ...
    def delete_note(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                ...
                self.display_linked_notes() # Explicitly refresh linked notes display
            else:
    ...
    def handle_ai_generation_finished(self, generated_notes):
        ...
        if generated_notes:
            ...
            self.display_linked_notes() # Refresh linked notes after generation and linking
        else:
    ...
    def open_selected_note_from_link(self, item):
        selected_note_id = item.data(Qt.UserRole)
        if selected_note_id:
            # Find the display title for the selected note ID
            display_title = None
            ...
            if display_title:
                # Simulate clicking the item in the main notes list
                # This will trigger open_selected_note and load the content
    ...
    def _show_linked_note_context_menu(self, position):
        item = self.linked_notes_list_widget.itemAt(position)
        if item:
            self.linked_notes_list_widget.setCurrentItem(item)
            context_menu = QMenu(self)
            unlink_action = context_menu.addAction("Unlink Note")
    ...
    def display_linked_notes(self):
        ...
        if self.current_note_id:
            linked_note_ids = self.db_manager.get_note_links(self.current_note_id)
            if linked_note_ids:
                for linked_id in linked_note_ids:
                    # Get note metadata to display title
                    note_data = self.db_manager.get_note(linked_id)
                    if note_data:
                        linked_title = note_data[1] # title is at index 1
                        item = QListWidgetItem(linked_title)
                        item.setData(Qt.UserRole, linked_id) # Store the actual note ID
                        self.linked_notes_list_widget.addItem(item)
            else:
                self.linked_notes_list_widget.clear()
                self.linked_notes_list_widget.addItem("No linked notes.")
        else:
            self.linked_notes_list_widget.clear()
            self.linked_notes_list_widget.addItem("Select a note to see its links.")

    def unlink_note(self):
        selected_items = self.linked_notes_list_widget.selectedItems()
        if not selected_items:
            QMessageBox.information(self, "Unlink Note", "Please select a linked note to unlink.")
            return

        linked_note_id_to_unlink = selected_items[0].data(Qt.UserRole)
        linked_note_title_to_unlink = selected_items[0].text()
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                ...
                self.display_linked_notes() # Refresh the linked notes list
            else:
